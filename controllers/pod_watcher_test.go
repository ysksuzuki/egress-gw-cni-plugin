...
func makePodWithHostNetwork(name string, ips []string, egresses map[string]string) {
	...
	for k, v := range egresses {
		pod.Annotations["egress.ysksuzuki.com/"+k] = v
	}
...
# HELP egressgw_egress_client_pod_count the number of client pods which use this egress
# TYPE egressgw_egress_client_pod_count gauge
egressgw_egress_client_pod_count{egress="egress2",namespace="internet"} %d
`, clientPodCount))
	return testutil.GatherAndCompare(metrics.Registry, expected, "egressgw_egress_client_pod_count")
}
...
var _ = Describe("Pod watcher", func() {
	...
	It("should check Pod replacement", func() {
		...
		pod1.Annotations = map[string]string{"egress.ysksuzuki.com/internet": "egress2"}
	...
	It("should not delete a peer that another pod is reusing", func() {
		...
		// egress-gw deletes the peer when the job pod completed
		Eventually(func() bool {
			return reflect.DeepEqual(ft.GetPeers(), map[string]bool{
				"10.1.1.2": true,
				"fd01::2":  true,
		...
		// egress-gw doesn't delete the peer that the job pod used because another pod is reusing it
		Consistently(func() bool {
			return reflect.DeepEqual(ft.GetPeers(), map[string]bool{
				"10.1.1.2": true,
				"fd01::2":  true,
	...
	It("should not delete a peer when another pod accidentally hits the same one", func() {
		...
		// egress-gw doesn't delete the peer of another pod
		Consistently(func() bool {
			return reflect.DeepEqual(ft.GetPeers(), map[string]bool{
				"10.1.1.2": true,
				"fd01::2":  true,
